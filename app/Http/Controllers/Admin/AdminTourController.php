...
class AdminTourController extends Controller
{
    ...
    public function create_submit(Request $request)
    {
        $request->validate([
            ...
            'tour_start_date.required'  => 'Datum početka ture je obavezan.',
            'tour_end_date.required'    => 'Datum završetka ture je obavezan.',
            'booking_end_date.required' => 'Datum završetka rezervacija je obavezan.',
            'total_seat.required'       => 'Unesite broj sedišta.',
        ]);
        ...
        return redirect()->route('admin_tour_index')->with('success', 'Tura je uspešno kreirana!');
    }
    ...
    public function edit_submit(Request $request, $id)
    {
        ...
        return redirect()->route('admin_tour_index')->with('success', 'Tura je uspešno ažurirana!');
    }
    ...
    public function delete($id)
    {
        ...
        if($total > 0) {
            return redirect()->back()->with('error', 'Ova tura već ima rezervacije i ne može biti obrisana.');
        }

        $obj = Tour::where('id',$id)->first();
        $obj->delete();
        return redirect()->route('admin_tour_index')->with('success', 'Tura je uspešno obrisana!');
    }
    ...
    public function tour_booking_delete($id)
    {
        ...
        $obj->delete();
        return redirect()->back()->with('success', 'Rezervacija je uspešno obrisana!');
    }
    ...
    public function tour_booking_approve($id)
    {
        ...
        return redirect()->back()->with('success', 'Rezervacija je uspešno odobrena!');
    }
}
